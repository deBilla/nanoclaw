...
registerChannelAdapter('whatsapp', {
  factory: () => {
    ...
    /** Transcribe an audio file via the local Whisper sidecar. Returns null if unavailable. */
    async function transcribeAudio(filePath: string): Promise<string | null> {
      ...
      try {
        ...
        form.append('audio_file', new Blob([buffer]), path.basename(filePath));
        const res = await fetch(`${whisperUrl}/asr?encode=true&task=transcribe&output=txt`, {
          method: 'POST',
          body: form,
      ...
      sock.ev.on('messages.upsert', async ({ messages }) => {
        for (const msg of messages) {
          try {
            ...
            // Transcribe voice notes / audio messages if Whisper sidecar is running.
            // On failure, replace the raw attachment with an inline note so the agent
            // never sees an inaccessible file path and knows exactly what to do.
            if (normalized.audioMessage) {
              const audioIdx = attachments.findIndex((a) => a.type === 'audio');
              if (audioIdx !== -1) {
                const transcript = await transcribeAudio(path.join(DATA_DIR, attachments[audioIdx].localPath));
                if (transcript) {
                  content = content ? `${content}\n[Voice]: ${transcript}` : `[Voice]: ${transcript}`;
                  attachments.splice(audioIdx, 1); // attachment consumed by transcript
                  log.info('Audio transcribed', { chars: transcript.length });
                } else {
                  content = content
                    ? `${content}\n[Voice message — transcription unavailable, ask the user to type]`
                    : '[Voice message — transcription unavailable, ask the user to type]';
                  attachments.splice(audioIdx, 1); // remove raw audio path; agent cannot access it
                }
              }
            }
